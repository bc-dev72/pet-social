package service;

import java.util.ArrayList;
import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

import commons.model.account.AccountDetails;
import repo.AccountDataRepo;
import repo.AccountDetailsRepo;
import rest.controller.request.accounts.AccountUpdateRequest;
import rest.controller.response.accounts.ProfileUpdateResponse;
import security.util.TokenAccountData;
import service.error.ServiceError;
import util.thread.DataCache;
import util.thread.UpdaterProtection;

@Service
public class AccountDataService {

	@Autowired
	private AccountDataRepo accountDataRepo;
	
	@Autowired
	private AccountDetailsRepo accountDetailsRepo;
	
	public ProfileUpdateResponse updateProfile(TokenAccountData tokenData, AccountUpdateRequest updateRequest) throws ServiceError {
		System.out.println("Worked");
		
		getAccess(tokenData.getAccountId());
		
		if(updateRequest.getBio().length() > 500)
			throw new ServiceError("Bio too long, please make it less than 500 characters"); 
		
		AccountDetails details = accountDetailsRepo.findByAccountId(tokenData.getAccountId());
		if(updateRequest.getProfilePicture() != null && !updateRequest.getProfilePicture().equals(""))
			details.setProfilePicture(updateRequest.getProfilePicture());
		
		details.setBio(updateRequest.getBio());
		accountDetailsRepo.save(details);
		
		this.releaseAccess(tokenData.getAccountId());
		
		ProfileUpdateResponse response = new ProfileUpdateResponse();
		response.setBio(details.getBio());
		String profilePic = details.getProfilePicture();
		if(profilePic == null || profilePic.length() == 0) 
			profilePic="data:image/webp;base64,";
		response.setProfilePicture(profilePic);
		return response;
	}
	
	
	public List<String> followUser(TokenAccountData tokenData, String username) throws ServiceError {
		if(username.toLowerCase().trim().equals(tokenData.getUsername()))
			throw new ServiceError("You can't follow yourself");
		if(!accountDataRepo.existsByUsername(username))
			throw new ServiceError("Username does not exist");
		return changeFollower(tokenData, username, true);
		
	}
	
	public List<String> unfollowUser(TokenAccountData tokenData, String username) throws ServiceError {
		if(username.toLowerCase().trim().equals(tokenData.getUsername()))
			throw new ServiceError("You can't unfollow yourself");
		return changeFollower(tokenData, username, false);
	}
	
	private List<String> changeFollower(TokenAccountData tokenData, String username, boolean add) {
		getAccess(tokenData.getAccountId());
		
		AccountDetails details = accountDetailsRepo.findByAccountId(tokenData.getAccountId());
		if(add)
			details.getFollowingList().add(username);
		else
			details.getFollowingList().remove(username);
		accountDetailsRepo.save(details);
		
		DataCache.setData(DataCache.START_FOLLOWERS+tokenData.getAccountId(), details.getFollowingList());
		
		releaseAccess(tokenData.getAccountId());
		return new ArrayList<>(details.getFollowingList());
	}
	
	
	
	private void getAccess(String accountId) {
		boolean waiting = true;
		while(waiting) {
			waiting = UpdaterProtection.requestRelease("ACCOUNT ID", accountId, false);
			try {Thread.sleep(100);} catch (InterruptedException e) {}
		}
	}
	
	private void releaseAccess(String accountId) {
		UpdaterProtection.requestRelease("ACCOUNT ID", accountId, true);
	}
	
	
}
